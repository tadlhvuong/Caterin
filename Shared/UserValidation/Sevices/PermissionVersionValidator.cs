...
namespace Shared.UserValidation.Sevices
{
    public sealed class PermissionVersionValidator : IPermissionVersionValidator
    {
        public Task<UserValidationResult> ValidateAsync(UserValidationContext context, AppUser user, CancellationToken cancellationToken = default)
        {
            ...
            if (principal == null)
                return Task.FromResult(UserValidationResult.Fail(UserValidationError.PrincipalMissed));

            var claimValue = principal.FindFirstValue(ClaimConstants.PermissionVersion);

            if (string.IsNullOrWhiteSpace(claimValue))
                return Task.FromResult(UserValidationResult.Fail(UserValidationError.PermissionVersionMissed));

            if (!long.TryParse(claimValue, out var jwtVersion))
                return Task.FromResult(UserValidationResult.Fail(UserValidationError.PermissionVersionInvalid));

            if (jwtVersion != user.PermissionVersion)
                return Task.FromResult(UserValidationResult.Fail(UserValidationError.PermissionVersionChanged));
