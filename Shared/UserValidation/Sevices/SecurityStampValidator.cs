...
namespace Shared.UserValidation.Sevices
{
    public sealed class SecurityStampValidator : ISecurityStampValidator
    {
        public Task<UserValidationResult> ValidateAsync(UserValidationContext context, AppUser user, CancellationToken cancellationToken = default)
        {
            ...
            var tokenStamp = context.Principal.FindFirstValue(ClaimConstants.SecurityStamp);

            if (string.IsNullOrWhiteSpace(tokenStamp))
                return Task.FromResult(UserValidationResult.Fail(UserValidationError.SecurityStampMissed));

            if (!string.Equals(tokenStamp, user.SecurityStamp, StringComparison.Ordinal))
                return Task.FromResult(UserValidationResult.Fail(UserValidationError.SecurityStampChanged));
